...
def get_all_posts():
    ...
    return render_template("index.html", all_posts=posts, logged_in=current_user.is_authenticated,
                           id=current_user.get_id(), year=datetime.datetime.now().year)
...
def register():
    ...
    return render_template("register.html", form=form, year=datetime.datetime.now().year)
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You need to be login before you can make a comment")
            return redirect(url_for('login'))
        else:
    ...
    return render_template("post.html", post=requested_post, logged_in=current_user.is_authenticated,
                           id=current_user.get_id(), form=form, year=datetime.datetime.now().year)


@app.route("/about")
def about():
    return render_template("about.html", logged_in=current_user.is_authenticated, year=datetime.datetime.now().year)


@app.route("/contact")
def contact():
    return render_template("contact.html", logged_in=current_user.is_authenticated, year=datetime.datetime.now().year)
...
def edit_post(post_id):
    ...
    return render_template("make-post.html", logged_in=current_user.is_authenticated, form=edit_form,
                           year=datetime.datetime.now().year)
